...
- **산타, 루돌프, 썰매는 하나의 세트로 함께 유지**
  ...
  - **애니메이션**:
    - 왼쪽에서 오른쪽으로 이동 (santaX += 0.5)
    - 오른쪽 끝 도달 시 왼쪽에서 다시 시작 (x > 220 → x = -20)
  - **전체 세트의 y 위치는 변경 가능**
...
- **클릭 시 선물이 떨어지는 인터랙티브 기능**
  ...
  - **Particle 클래스** (폭발 이펙트):
    ...
    - 360도 모든 방향으로 랜덤 발사
    - 중력 효과 적용 (gravity: 0.15)
    - 페이드아웃 효과 (life: 1.0 → 0, fadeSpeed: 0.02)
    - 선물 생성 시: 6-8개 파티클
    - 땅 착지 시: 12-15개 파티클
...
- **5의 배수 선물 폭발 시 발동되는 폭죽 애니메이션**
  ...
  - **FireworkParticle 클래스** (폭발 파티클):
    - 파티클 개수: 50-70개
    - 파티클 속도: 2~5 범위 (더 멀리 퍼짐)
    - 파티클 크기: 1픽셀(70%), 2픽셀(30%)
    - 360도 모든 방향으로 랜덤 발사
    - 중력 효과: 0.08
    - 공기 저항: 0.98 (매 프레임마다 속도의 98%로 감소)
    - 페이드아웃: life 1.0 → 0, fadeSpeed 0.012 (더 오래 보임)
  - **발사 시스템**:
